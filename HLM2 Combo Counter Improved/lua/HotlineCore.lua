...
function HUDComboCounter:open_anim(panel)
    local speed = HMHCC:GetOption("open_speed") or 50
    panel:set_x(-150)
    local distance = 50 - panel:x()
    local timespan = 10/speed
    local elapsed = 0
    local panel_y = HMHCC:GetOption("panel_position") or 40
...
function HUDComboCounter:close_anim(panel)
    local speed = HMHCC:GetOption("close_speed") or 50
    ...
    local Combo_text_bg = self._combo_panel:child("Combo_text_bg")
    --[[while t > 0 do
        if not (self._kills == 0) then return end
...
function HUDComboCounter:OnKillshot()
    ...
    local kill_flash = HMHCC:GetOption("kill_flash") or false
    log("Kill Anim: " .. tostring(kill_anim))
    log("Kill Flash: " .. tostring(kill_flash))
